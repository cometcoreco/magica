...
<div class="conversation-area flex grow flex-col justify-between overflow-y-auto max-md:max-h-[70vh] max-sm:max-h-full"
    ...
    <form
                ...
                <div
                    ...
                    <div id="chat-options"
                        ...
                        <div class="flex items-center pointer-events-auto max-sm:pt-4">
                            <button type="button"
                                @if (env('APP_STATUS') == 'Demo') onclick="return toastr.info('This feature is disabled in Demo version.')" @else id="chat_add_image" @endif
        ...
        @if ($setting->hosting_type == 'high')
            <button
                @if (env('APP_STATUS') == 'Demo' && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf' || $category->slug == 'ai_chat_image')) onclick="return toastr.info('This feature is disabled in Demo version.')" @endif
                class="btn btn-primary h-[52px] w-[52px] shrink-0 rounded-full p-0 max-sm:h-10 max-sm:w-10"
                @if (env('APP_STATUS') == 'Demo' && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf' || $category->slug == 'ai_chat_image')) @else id="send_message_button" @endif form="chat_form">
        ...
        @else
            ...
            <button
                @if (env('APP_STATUS') == 'Demo' && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf'  || $category->slug == 'ai_chat_image')) onclick="return toastr.info('This feature is disabled in Demo version.')" @endif
                class="btn btn-primary h-[52px] w-[52px] shrink-0 rounded-full p-0 max-sm:h-10 max-sm:w-10"
                @if (env('APP_STATUS') == 'Demo' && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf' || $category->slug == 'ai_chat_image')) @else id="send_message_button" @endif type="button">
                <svg class="rtl:-scale-x-100" width="16" height="14" viewBox="0 0 16 14" fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg">
                    <path d="M0.125 14V8.76172L11.375 7.25L0.125 5.73828V0.5L15.875 7.25L0.125 14Z" />
                </svg>
